...
static void lc_init() {
    ...
    // ── Hook startup state ─────────────────────────────────────────────────────
    // Binary .metallib files (Arena Breakout, many non-UE games) are loaded in the
    // FIRST millisecond of process startup — long before the 3s auto-enable timer.
    // Starting hooks OFF means we miss every binary shader on first launch.
    //
    // We now start hooks ON by default. This is safe because:
    //   • No patches are active at startup (colorLibraries / activeColorHashes empty)
    //   • The library hooks only observe — they never build GPU pipelines at startup
    //   • All GPU-compiler-crash paths are already fixed (fmColorPatch / fmFlashPatch)
    //
    // Respect the user kill-switch (explicit NO saved to NSUserDefaults) so that
    // long-pressing to emergency-stop still works after a restart.
    NSUserDefaults *hookUD = [NSUserDefaults standardUserDefaults];
    id hookPref = [hookUD objectForKey:@"FMHooksEnabled"]; // nil → key missing (first launch)
    BOOL savedHooks = [hookUD boolForKey:@"FMHooksEnabled"];
    // Default YES (key absent = first launch or new game). Only NO if user kill-switched.
    gHooksEnabled = (hookPref == nil) ? YES : savedHooks;
