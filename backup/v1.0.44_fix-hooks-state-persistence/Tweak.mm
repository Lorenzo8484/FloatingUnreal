...
static void lc_init() {
    ...
    //
    // v1.0.44: leggiamo savedHooks QUI, prima del crash counter, così il blocco cc>1
    // può fare override locale senza mai scrivere NO su UserDefaults.
    // La crash flag FILE (segnali POSIX reali) scrive già NO permanente quando serve.
    NSUserDefaults *hookUD = [NSUserDefaults standardUserDefaults];
    BOOL savedHooks = [hookUD boolForKey:@"FMHooksEnabled"];
    {
        NSUserDefaults *ud = hookUD;
        NSInteger cc = [ud integerForKey:@"FMCrashCount"] + 1;
        [ud setInteger:cc forKey:@"FMCrashCount"];
        [ud synchronize];
        NSLog(@"[FM] CrashCount=%ld", (long)cc);
        if (cc > 1) {
            // GPU crash loop rilevato (GPU watchdog non genera POSIX signal → flag file vuoto).
            // v1.0.44: override LOCALE solo — NON scrivere NO su UserDefaults.
            // La preferenza utente rimane intatta: al prossimo lancio stabile (>45s)
            // il contatore si azzera e l'utente ritrova lo stato che aveva salvato.
            NSLog(@"[FM] ⚠️  CrashLoop (cc=%ld) — hooks OFF questa sessione (pref utente preservata)", (long)cc);
            savedHooks = NO;  // override locale: questa sessione parte sicura
            [ud removeObjectForKey:@"FMSavedPatches_v3"];
            [ud setInteger:0  forKey:@"FMCrashCount"];
            [ud synchronize];
        }
        // Reset crash counter after 45s of stable run.
    ...
    // ── Hook startup state ─────────────────────────────────────────────────────
    // gHooksEnabled: ripristinato da savedHooks (preferenza utente).
    // Safety net a due livelli:
    //   1. Crash flag file (segnali POSIX): scrive FMHooksEnabled=NO permanente se crash reale.
    //   2. Crash counter (GPU watchdog, non POSIX): override locale savedHooks=NO questa
    //      sessione senza toccare UserDefaults — al prossimo lancio stabile (<45s) l'utente
    //      ritrova il suo stato ON.
    gHooksEnabled      = savedHooks;  // ripristina preferenza utente
