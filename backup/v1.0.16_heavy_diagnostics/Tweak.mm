...
static void applyPatchesForEntry(ShaderEntry *entry, MTLCompileOptions *opts) {
    ...
    if (hasColor) {
        ...
        if (injectWorked) {
            ...
            fmDiag([NSString stringWithFormat:@"[C] compileDirect done lib=%s", lib ? "OK" : "nil"]);
            if (lib) {
            ...
            fmDiag(@"[B2] binary IR frag — using FM_CONST_FRAG");
...
static void buildVariantPipelineAsync(id device,
    ...
    fmDiag([NSString stringWithFormat:@"[J] calling Metal pipeline d.vFn=%@ d.fFn=%@ d.smp=%lu async=%s",
    ...
    if (origNewPipelineAsync) {
        origNewPipelineAsync(device,
            ...
            ^(id<MTLRenderPipelineState> ps, NSError *err) {
                fmDiag([NSString stringWithFormat:@"[K] async done ps=%s err=%@", ps ? "OK" : "nil", err.localizedDescription ?: @"no-err"]);
                if (completion) completion(ps);
            });
    } else {
        // Fallback: sync build on caller's background queue
        fmDiag(@"[J2] using sync fallback origNewPipeline");
        NSError *err2 = nil;
        id<MTLRenderPipelineState> ps2 = origNewPipeline(device,
            @selector(newRenderPipelineStateWithDescriptor:error:), d, &err2);
        fmDiag([NSString stringWithFormat:@"[K2] sync done ps=%s err=%@", ps2 ? "OK" : "nil", err2.localizedDescription ?: @"no-err"]);
