...
static void lc_init() {
    ...
    //
    // v1.0.39: gCaptureSourceLibs=NO al startup.
    // Con YES, hooked_newLibraryWithSource scatta da thread Metal e chiama
    // method_setImplementation (dentro dispatch_once) dal background → crash ObjC runtime.
    // gCaptureSourceLibs=YES viene abilitato da fmSetHooksEnabled() quando l'utente preme ON.
    gHooksEnabled      = NO;   // patch OFF — attiva manualmente dal menu
    gCaptureSourceLibs = NO;   // NO al startup — abilitato con ON dal menu
