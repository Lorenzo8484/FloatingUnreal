...
static void hookEncClass(Class cls) {
    if (!cls) return;
    NSLog(@"[FM-DIAG] hookEncClass: trying %s", class_getName(cls));
    ...
    if (mp) {
        ...
        if (cur != (IMP)hooked_setRenderPipelineState) {
            ...
            NSLog(@"[FM-DIAG] hookEncClass: INSTALLED setRenderPipelineState on %s origSetPipe=%p",
                  class_getName(cls), (void*)origSetPipe);
        } else {
            NSLog(@"[FM-DIAG] hookEncClass: setRenderPipelineState already our hook on %s", class_getName(cls));
        }
    } else {
        NSLog(@"[FM-DIAG] hookEncClass: setRenderPipelineState NOT FOUND on %s", class_getName(cls));
    }
