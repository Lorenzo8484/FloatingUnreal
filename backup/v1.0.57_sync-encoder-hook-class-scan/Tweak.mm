...
static void installHooks(id<MTLDevice> device) {
    ...
    // 3c. Scan ALL ObjC classes that conform to MTLRenderCommandEncoder and hook them NOW.
    // This replaces the lazy hookEncClass-via-renderCommandEncoder path which fails when the
    // Metal framework uses private encoder subclasses not reachable through our hook chain.
    // hookEncClass is safe to call on main thread (we ARE on main — __attribute__((constructor))).
    {
        Protocol *encProto = objc_getProtocol("MTLRenderCommandEncoder");
        if (encProto) {
            unsigned int count = 0;
            Class *classes = objc_copyClassList(&count);
            NSUInteger found = 0;
            for (unsigned int i = 0; i < count; i++) {
                if (class_conformsToProtocol(classes[i], encProto)) {
                    hookEncClass(classes[i]);
                    found++;
                }
            }
            free(classes);
            NSLog(@"[FM] installHooks: scanned %u classes, hooked %lu MTLRenderCommandEncoder", count, (unsigned long)found);
        }
    }
