...
static void fmBuildReflectionForDesc(id device, MTLRenderPipelineDescriptor *desc, NSString *pairKey);
...
static NSMutableDictionary *pipelineReflections  = nil; // @"fHash|vHash" → NSDictionary (serialized MTLRenderPipelineReflection)
static NSMutableSet        *gReflectionBuiltPairs = nil; // dedup: avoid rebuilding reflection for same pair
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)),
            ...
            // for EVERY pipeline the game uses, regardless of patching state.
            {
                NSString *reflectKey = [NSString stringWithFormat:@"%@|%@",
                    ...
                    vHashCopy ? [NSString stringWithFormat:@"%lx", vHashCopy.unsignedIntegerValue] : @"0"];
                BOOL doReflect = NO;
                @synchronized(gReflectionBuiltPairs) {
                    if (![gReflectionBuiltPairs containsObject:reflectKey]) {
                        [gReflectionBuiltPairs addObject:reflectKey];
                        doReflect = YES;
                    }
                }
...
void fmClearAllShaderPatches(void) {
    @synchronized(gHookLock) {
        ...
        [pipelineReflections  removeAllObjects];
        @synchronized(gReflectionBuiltPairs) { [gReflectionBuiltPairs removeAllObjects]; }
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
// ── Pipeline Reflection API (GPU metadata extraction) ─────────────────────────
// Uses Metal MTLRenderPipelineReflection to extract buffer/uniform struct layouts
// directly from the GPU driver. Works for ALL shaders (MSL, binary .metallib, obfuscated).
// This bypasses MSL source parsing entirely — we get the REAL struct member names,
// offsets, and types as compiled by the Metal compiler.

static NSString *fmReflectionArgTypeName(MTLDataType t) {
    switch (t) {
        ...
        default: return [NSString stringWithFormat:@"type%lu", (unsigned long)t];
    }
}

// Overload for MTLArgumentType (different enum, same values for shared types)
static NSString *fmArgTypeName(MTLArgumentType t) {
    switch (t) {
        ...
        case MTLArgumentTypeThreadgroupMemory: return @"threadgroup";
        case MTLArgumentTypeVisibleFunctionTable: return @"visiblefn";
        case MTLArgumentTypeIntersectionFunctionTable: return @"intersectionfn";
        case MTLArgumentTypePrimitiveAccelerationStructure: return @"accel";
        case MTLArgumentTypeInstanceAccelerationStructure: return @"instance_accel";
        default: return [NSString stringWithFormat:@"arg%lu", (unsigned long)t];
    }
}

// Build a readable struct-body string from MTLStructType (reflection data).
static NSString *fmStructFromReflection(MTLStructType *st, NSString *structName) {
    if (!st) return nil;
    NSMutableString *s = [NSMutableString string];
    [s appendFormat:@"struct %@ {\n", structName ?: @"unknown"];
    for (MTLStructMember *m in st.members) {
        NSString *typeName = fmReflectionArgTypeName(m.dataType);
        if (m.dataType == MTLDataTypeArray && m.arrayType) {
            typeName = [NSString stringWithFormat:@"%@[%lu]",
                fmReflectionArgTypeName(m.arrayType.elementType),
                (unsigned long)m.arrayType.arrayLength];
        }
        [s appendFormat:@"    %@ %@; // offset=%lu\n",
            typeName, m.name ?: @"?", (unsigned long)m.offset];
    }
    [s appendString:@"};\n"];
    return s;
}

// Serialize reflection data into a storable NSDictionary.
static NSDictionary *fmSerializeReflection(MTLRenderPipelineReflection *ref) {
    if (!ref) return nil;
    NSMutableDictionary *d = [NSMutableDictionary dictionary];

    // Vertex arguments
    NSMutableArray *vArgs = [NSMutableArray array];
    for (MTLArgument *arg in ref.vertexArguments) {
        if (!arg.isActive) continue;
        NSMutableDictionary *ad = [NSMutableDictionary dictionary];
        ad[@"name"]  = arg.name ?: @"?";
        ad[@"index"] = @(arg.index);
        ad[@"type"]  = fmArgTypeName(arg.type);
        if (arg.type == MTLArgumentTypeTexture)
            ad[@"textureType"] = [NSString stringWithFormat:@"tex%lu", (unsigned long)arg.textureType];
        if (arg.bufferStructType) {
            ad[@"structName"] = arg.name ?: @"?";
            ad[@"structBody"] = fmStructFromReflection(arg.bufferStructType, arg.name) ?: @"";
        }
        [vArgs addObject:ad];
    }
    d[@"vertexArgs"] = vArgs;
    ...
    for (MTLArgument *arg in ref.fragmentArguments) {
        if (!arg.isActive) continue;
        NSMutableDictionary *ad = [NSMutableDictionary dictionary];
        ad[@"name"]  = arg.name ?: @"?";
        ad[@"index"] = @(arg.index);
    ...
    d[@"fragmentArgs"] = fArgs;

    return d;
}

// Build reflection for a (fHash, vHash) pair from a stored descriptor.
// Called from buildVariantPipelineAsync to avoid blocking the render thread.
static void fmBuildReflectionForDesc(id device, MTLRenderPipelineDescriptor *desc,
                                      NSString *pairKey) {
    if (!desc || !device) return;
    // Use MTLPipelineOptionArgumentInfo to get buffer struct layouts
    MTLRenderPipelineDescriptor *d = [desc copy];
    MTLRenderPipelineReflection *refOut = nil;
    NSError *err = nil;
    id<MTLRenderPipelineState> rp = [device
        newRenderPipelineStateWithDescriptor:d
        options:MTLPipelineOptionArgumentInfo
        reflection:&refOut
        error:&err];
    if (!rp || err) {
        fmLog([NSString stringWithFormat:@"[REFL ERR] %@ → %@", pairKey,
            err.localizedDescription ?: @"pipeline nil"]);
        return;
    }
    NSDictionary *ser = fmSerializeReflection(refOut);
    if (ser) {
        @synchronized(gHookLock) {
            pipelineReflections[pairKey] = ser;
        }
        NSUInteger vc = [ser[@"vertexArgs"] count];
        NSUInteger fc = [ser[@"fragmentArgs"] count];
        fmLog([NSString stringWithFormat:@"[REFL ✓] %@ vArgs=%lu fArgs=%lu",
            pairKey, (unsigned long)vc, (unsigned long)fc]);
    }
    // rp is discarded — we only needed the reflection
}

// Public accessor for ShaderPage to get reflection data for a hash pair.
NSDictionary *fmGetReflectionForPair(NSString *pairKey) {
    if (!pairKey) return nil;
    @synchronized(gHookLock) {
        return pipelineReflections[pairKey];
    }
}

// Find the best reflection struct for a given shader hash.
// Scans all stored reflections looking for entries whose pairKey contains this hash.
// Returns a dict with keys: @"structName", @"structBody", @"argName", @"argType", @"from" (@"vertex" or @"fragment").
NSDictionary *fmGetBestReflectionForHash(NSNumber *hashKey) {
    if (!hashKey) return nil;
    NSString *hashStr = [NSString stringWithFormat:@"%lx", hashKey.unsignedIntegerValue];
    @synchronized(gHookLock) {
        for (NSString *pairKey in pipelineReflections) {
            if (![pairKey containsString:hashStr]) continue;
            NSDictionary *ref = pipelineReflections[pairKey];
            // Prefer fragment args with buffer struct (these are uniforms)
            for (NSDictionary *arg in ref[@"fragmentArgs"]) {
                if (arg[@"structBody"] && [arg[@"structBody"] length] > 4) {
                    return @{@"structName": arg[@"structName"] ?: @"?",
                             @"structBody": arg[@"structBody"],
                             @"argName": arg[@"name"] ?: @"?",
                             @"argType": arg[@"type"] ?: @"?",
                             @"from": @"fragment"};
                }
            }
            // Fall back to vertex args
            for (NSDictionary *arg in ref[@"vertexArgs"]) {
                if (arg[@"structBody"] && [arg[@"structBody"] length] > 4) {
                    return @{@"structName": arg[@"structName"] ?: @"?",
                             @"structBody": arg[@"structBody"],
                             @"argName": arg[@"name"] ?: @"?",
