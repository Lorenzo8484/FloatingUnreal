...
NSSet *fmCopyLiveActiveHashes(void) {
    @synchronized(gHookLock) {
        NSSet *snap = [gLiveActiveHashes copy];
        // Clear after snapshot: next LIVE press accumulates hashes from this point forward.
        // (Clearing per-frame in hooked_commandBuffer caused empty snapshots when pressed
        //  between frame-clear and the next setRenderPipelineState calls.)
