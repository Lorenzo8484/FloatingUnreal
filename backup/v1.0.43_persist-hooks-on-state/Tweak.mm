...
static _Atomic(BOOL) gCaptureBinaryLibs = NO;  // v1.0.42: OFF — binary metallib non mostrate in UI (no sorgente MSL, non patchabili)
...
static NSNumber *lateTagLibrary(id<MTLLibrary> lib) {
    ...
    if (isNew) {
        ...
        // v1.0.42: NON aggiungere alla UI — librerie binary (non-MSL) non hanno sorgente patchabile.
        // L'hash è calcolato e registrato in capturedSources per il pipeline tracking interno,
        // ma l'utente vede solo shader MSL con sorgente reale.
        (void)srcCopy; (void)dispName;
    }
    return hKey;
}
...
static id<MTLRenderCommandEncoder> hooked_renderCommandEncoder(id self, SEL _cmd,
    ...
    // v1.0.33: encHookOnce BEFORE gHooksEnabled — hookEncClass must run at first encoder
    // regardless of hook state so origSetPipe is valid when user later enables hooks.
    id<MTLRenderCommandEncoder> enc = origRenderEnc(self, _cmd, desc);
    // v1.0.40: dispatch hookEncClass to MAIN THREAD — method_setImplementation from
    // render thread crashes ObjC runtime. !origSetPipe fast-check avoids dispatching every frame.
    if (enc && !origSetPipe) {
        Class encCls = object_getClass(enc);
        static dispatch_once_t encHookOnce;
        dispatch_async(dispatch_get_main_queue(), ^{
            dispatch_once(&encHookOnce, ^{ hookEncClass(encCls); });
        });
    }
...
static NSString *fmAirReflect(id<MTLLibrary> lib, id<MTLDevice> device,
    ...
    for (NSString *fnName in funcNames) {
        ...
        if (block.length > 0) {
            ...
            // Fallback: just note the function name
...
static id<MTLLibrary> hooked_newLibraryWithData(id self, SEL _cmd,
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (floatingMenu) [floatingMenu captureShaderWithName:dispName source:placeholderSrc error:nil];
    });
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (floatingMenu) [floatingMenu captureShaderWithName:dispName source:reflected error:nil];
        });
    });
...
hooked_newLibraryWithURL(id self, SEL _cmd, NSURL *url, NSError **error) {
    ...
    if (isNew) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (floatingMenu) [floatingMenu captureShaderWithName:nm source:ph error:nil];
        });
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
            ...
            @synchronized(gHookLock) { capturedSources[hKey] = reflected; }
            dispatch_async(dispatch_get_main_queue(), ^{
                if (floatingMenu) [floatingMenu captureShaderWithName:nm source:reflected error:nil];
            });
        });
    }
    return lib;
...
static id<MTLCommandBuffer> hooked_commandBuffer(id self, SEL _cmd) {
    ...
    // Clear live-active set only when hooks are enabled
    if (!gHooksEnabled) return buf;
...
static void lc_init() {
    ...
    NSString *libDir = nil;
    {
        ...
        if (flagContent.length > 0) {
            NSLog(@"[FM] ⚠️ Crash flag rilevato (sessione precedente crashata) — hooks resettati a OFF");
            [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"FMHooksEnabled"];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
    ...
    // This breaks the cycle: flash crash → restore flash → crash → repeat.
    {
        ...
        if (cc > 1) {
            // Previous launch crashed before the 6s stable mark — GPU driver crashes
            // do NOT fire POSIX signals, so the crash-flag file stays empty. This
            // counter is the only reliable way to detect those crashes.
            // FORCE hooks OFF and wipe ALL shader patches so the next launch is safe.
            NSLog(@"[FM] ⚠️  CrashLoop rilevato (cc=%ld) — hooks FORZATI OFF, patch azzerate", (long)cc);
            [ud setBool:NO forKey:@"FMHooksEnabled"];
            [ud removeObjectForKey:@"FMSavedPatches_v3"];
            [ud setInteger:0  forKey:@"FMCrashCount"];
            [ud synchronize];
        }
        // Reset crash counter after 6s of stable run.
        // NOTE: gCrashFlagFD stays open for the ENTIRE session — the signal handler
        // must be able to write the crash marker even during a late-match crash (e.g.
        // at T+20min). The file is truncated (cleared) at the START of each launch,
        // so an empty file on next boot means "clean exit", non-empty means "crashed".
        // v1.0.36: finestra estesa a 45s — i crash UE4 avvengono a t+22s,
        // ben oltre i 6s precedenti. Con 45s catturiamo quei crash col contatore.
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(45.0 * NSEC_PER_SEC)),
            ...
            [[NSUserDefaults standardUserDefaults] synchronize];
            NSLog(@"[FM] ✓ 45s stabile — CrashCount resettato");
        });
    }
    ...
    // v1.0.37: pipeline hooks SEMPRE OFF al cold-start.
    // UE4 compila le sue pipeline nei primi 20-30s dopo il launch: abilitare
    // gHooksEnabled durante quella finestra causa GPU watchdog SIGKILL.
    // Il crash loop guard (cc > 1) non scatta perché i crash avvengono a t+22s,
    // DOPO il reset del contatore a 6s (ora esteso a 45s).
    // L'auto-enable da FloatingMenu a 40s è stato RIMOSSO in v1.0.37 — same
    // root cause: se la compilazione pipeline dura oltre 30s, l'auto-enable a 40s
    // fa scattare il crash identico.
    // Solo l'utente può abilitare manualmente le patch (dopo che il gioco è carico).
    //
    // v1.0.43: gHooksEnabled ripristinato dal valore salvato (savedHooks).
    // Safety net: il crash counter sopra (cc > 1, finestra 45s) già resetta FMHooksEnabled=NO
    // in UserDefaults PRIMA di questa lettura — se il lancio precedente è crashato entro 45s,
    // savedHooks è già NO qui → hook OFF → lancio sicuro.
    // Se invece il gioco gira stabile >45s con hooks ON, il counter si azzera → al prossimo
    // lancio savedHooks=YES → hook ON → comportamento persistente che l'utente si aspetta.
    gHooksEnabled      = savedHooks;  // ripristina preferenza utente (protetto dal crash counter)
    gCaptureSourceLibs = YES;         // MSL capture sempre attiva al startup (sicura con v1.0.40)
    ...
    [[NSNotificationCenter defaultCenter]
        ...
        // Replay everything now so the user sees the full shader list immediately.
        {
            ...
            for (NSNumber *k in replayKeys) {
                ...
                if (nm && src) {
                    [floatingMenu captureShaderWithName:nm source:src error:nil];
                    n++;
                }
            }
            if (n > 0)
