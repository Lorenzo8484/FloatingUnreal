...
- (void)setupShaderPage {
    ...
    // v1.0.37: hooks SEMPRE OFF al cold-start, auto-enable RIMOSSO.
    // UE4 compila pipeline per 20-30s (e oltre): qualsiasi auto-enable causa
    // GPU watchdog SIGKILL. L'utente deve premere ⚡ ON manualmente dopo
    // che il gioco è completamente caricato.
    [self.shaderPage applyHookSwitchState:NO];
    BOOL savedHooks = [[NSUserDefaults standardUserDefaults] boolForKey:@"FMHooksEnabled"];
    if (savedHooks) {
        [self addLog:@"[SAFE] Hooks disabilitati per sicurezza UE4 — premi ⚡ ON dopo il loading"];
    } else {
        [self addLog:@"[INFO] Hooks OFF — premi ⚡ ON per attivare le patch"];
    }
}
