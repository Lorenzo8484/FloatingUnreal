...
static void lc_init() {
    ...
    // v1.0.36: SEMPRE NO al cold-start — UE4 compila pipeline per 20-30s dopo il launch.
    // Se gHooksEnabled=YES durante quella finestra, il GPU watchdog SIGKILL il processo
    // e il crash loop guard (cc > 1) non scatta perché i crash avvengono a t+22s,
    // DOPO il reset del contatore a 6s.
    // La preference viene rispettata solo dopo 40s (finestra sicura).
    gHooksEnabled      = NO;
    gCaptureSourceLibs = NO; // MSL capture segue gHooksEnabled per coerenza
