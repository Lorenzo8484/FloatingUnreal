...
- (void)setupShaderPage {
    ...
    // v1.0.36: hooks sempre OFF al cold-start (fix crash loop UE4 pipeline).
    // Se la sessione precedente aveva hooks ON, auto-abilita dopo 40s (finestra sicura).
    // UE4 compila le sue pipeline nei primi 20-30s — abilitare prima causa SIGKILL.
    [self.shaderPage applyHookSwitchState:NO];
    BOOL savedHooks = [[NSUserDefaults standardUserDefaults] boolForKey:@"FMHooksEnabled"];
    if (savedHooks) {
        [self addLog:@"[INFO] ⏳ Hooks in attesa 40s (finestra sicura UE4)..."];
        __weak FloatingMenu *wm = self;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(40.0 * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            FloatingMenu *me = wm;
            if (!me) return;
            fmSetHooksEnabled(YES);
            [me.shaderPage applyHookSwitchState:YES];
            [me addLog:@"[INFO] ⚡ Hooks ON — auto-abilitati (40s finestra sicura)"];
        });
    }
}
