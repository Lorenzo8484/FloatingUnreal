...
static void lc_init() {
    ...
    //
    // v1.0.41: gCaptureSourceLibs=YES al startup — ora sicuro.
    // v1.0.40 ha fixato il crash: funcHookOnce usa dispatch_async(main_queue, ...)
    // quindi method_setImplementation scatta sempre sul main thread, non dal thread Metal.
    // Con YES si catturano tutti gli shader MSL compilati durante il loading di UE4,
    // prima che l'utente prema ON — la lista shader è completa all'apertura del menu.
    gHooksEnabled      = NO;   // patch OFF — attiva manualmente dal menu
    gCaptureSourceLibs = YES;  // MSL capture attiva al startup (sicura con v1.0.40)
