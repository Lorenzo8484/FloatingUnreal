...
// Forward decl (defined later in Reflection API section)
...
static id<MTLRenderPipelineState> hooked_newRenderPipelineState(id self, SEL _cmd,
    ...
    // in-flight builds.
    {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)),
            ...
            // Runs even when no patches active - extracts real shader metadata
            {
...
// Overload for MTLArgumentType (different enum)
...
static void fmBuildReflectionForDesc(id device, MTLRenderPipelineDescriptor *desc,
    ...
    if (ser) {
        ...
        fmLog([NSString stringWithFormat:@"[REFL OK] %@ vArgs=%lu fArgs=%lu",
            pairKey, (unsigned long)vc, (unsigned long)fc]);
    }
}

// Public accessor for ShaderPage
...
NSDictionary *fmGetBestReflectionForHash(NSNumber *hashKey) {
    ...
    NSString *hs = [NSString stringWithFormat:@"%lx", hashKey.unsignedIntegerValue];
    @synchronized(gHookLock) {
        for (NSString *pk in pipelineReflections) {
            if (![pk containsString:hs]) continue;
            NSDictionary *ref = pipelineReflections[pk];
